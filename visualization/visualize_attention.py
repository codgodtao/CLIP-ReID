"""
CLIP-REID Attention Visualization - GUI Interface
...
Usage:
    python visualization/visualize_attention.py --config_file configs/person/vit_clipreid.yml --model_weight path/to/weight.pth
"""
...
class AttentionVisualizationApp:
    ...
    def _create_widgets(self):
        ...
        self.original_fig = plt.figure(figsize=(5, 5))
        ...
        self.heatmap_fig = plt.figure(figsize=(5, 5))
        self.heatmap_ax = self.heatmap_fig.add_subplot(111)
        self.heatmap_canvas = FigureCanvasTkAgg(self.heatmap_fig, middle_frame)
        ...
        self.overlay_fig = plt.figure(figsize=(5, 5))
        self.overlay_ax = self.overlay_fig.add_subplot(111)
        self.overlay_canvas = FigureCanvasTkAgg(self.overlay_fig, right_frame)
    ...
    def load_image(self):
        ...
        if file_path:
            try:
                ...
                self.original_ax.imshow(self.current_image)
                self.original_ax.axis('off')
    ...
    def generate_visualization(self):
        ...
        try:
            ...
            if heatmap is not None:
                self.heatmap_ax.clear()
                self.heatmap_ax.imshow(heatmap)
                self.heatmap_ax.axis('off')
                self.heatmap_canvas.draw()
    ...
    def save_result(self):
        ...
        if file_path:
            try:
                ...
                self.heatmap_fig.canvas.draw()
                heatmap_img = np.frombuffer(self.heatmap_fig.canvas.tostring_rgb(), dtype=np.uint8)
                w, h = self.heatmap_fig.canvas.get_width_height()
                heatmap_img = heatmap_img.reshape(h, w, 3)
                ...
                self.overlay_fig.canvas.draw()
                overlay_img = np.frombuffer(self.overlay_fig.canvas.tostring_rgb(), dtype=np.uint8)
                overlay_img = overlay_img.reshape(h, w, 3)
